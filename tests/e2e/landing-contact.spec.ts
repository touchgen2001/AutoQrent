...
test('landing to contact form flow works', async ({ page }) => {
  ...
  const quickDemoButton = page.getByRole('button', { name: 'WhatsApp Demo Talebi Oluştur' })
  await expect(quickDemoButton).toBeDisabled()
  await page.locator('#quickDemoGalleryName').fill('Test Galeri')
  await page.locator('#quickDemoVehicleCount').selectOption('16 - 50 araç')
  ...
  await page.locator('#galleryName').fill('Test Galeri')
  await page.locator('#vehicleCount').selectOption('16-50')
  await page.locator('#currentManagement').selectOption('excel-whatsapp')
