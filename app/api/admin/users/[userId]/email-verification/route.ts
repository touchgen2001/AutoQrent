...
import { verifyAdminUserEmail } from '@/lib/server/admin-users-repository'
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const parsed = emailVerificationSchema.safeParse(payload)
    ...
    const result = await verifyAdminUserEmail({
      userId,
